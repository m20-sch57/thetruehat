...
- `gameHistory (array)`
  - `speaker` Кто объяснял на соотв. ходу
  - `listenr` Кто отгадывал на соотв. ходу
  - `words (array)` - Список слов на соотв. ходу
    + `word (string)` - Если `wordState != not-explained`. Cлово.
    + `wordState (string)` - состояние слова.
      * `explained` - слово было объяснено.
      * `not-explained` - слово не было объяснене
...
__15.__ `io` <a name="cStartGame">`cStartGame`</a> - функция, начинающая игру. Используется только хостом.

---

__16.__ `io` <a name="sGameStarted">`sGameStarted`</a> - сигнал, посылаемый сервером всем клиентам, когда игра началась.
...
__17.__ `io` <a name="sNextTurn">`sNextTurn`</a> - сигнал, посылаемый сервером всем клиентам в начале хода.
...
__18.__ `io` <a name="cListenerReady">`cListenerReady`</a> - функция, обозначающая, что `listener` готов угадывать слова. Используется только `listener`-ом.

---

__19.__ `io` <a name="cSpeakerReady">`cSpeakerReady`</a> - функция, обозначающая, что `speaker` готов объяснять слова. Используется только `speaker`-ом.

---

__20.__ `io` <a name="sExplanationStarted">`sExplanationStarted`</a> - сигнал, посылаемый сервером всем клиентам перед началом объяснения.

- `startTime (int)` - время начала объяснения (в мс, в стандарте UTC+0 с начала Эпохи).

---

__21.__ `io` <a name="sNewWord">`sNewWord`</a> - сигнал, посылаемый сервером `speaker`-у, когда нужно сообщить слово для объяснения.

- `word` - слово для объяснения.

---

__22.__ `io` <a name="cEndWordExplanation">`cEndWordExplanation`</a> - функция, обозначающая, что `speaker` закончил объяснять текущее слово.
...
__23.__ `io` <a name="sWordExplanationEnded">`sWordExplanationEnded`</a> - сигнал, посылаемый всем клиентам, когда объяснение одного слова закончилось.
...
__24.__ `io` <a name="sExplanationEnded">`sExplanationEnded`</a> - сигнал, посылаемый всем клиентам в конце объяснения слов.

- `wordsCount (int)` - Если `settings.termCondition = words`. Количество слов в шляпе.
---

__25.__ `io` <a name="sWordsToEdit">`sWordsToEdit`</a> - сигнал, посылаемый `speaker`-у со списком слов для редактирования.
...
__26.__ `io` <a name="cWordsEdited">`cWordsEdited`</a> - сигнал, посылаемый `speaker`-ом, когда все правки внесены.
...
__27.__ `io` <a name="cEndGame">`cEndGame`</a> - сигнал, посылаемый `host`-ом, когда он хочет преждевременно закончить игру

---

__28.__ `io` <a name="sGameEnded">`sGameEnded`</a> - сигнал, посылаемый всем клиентам в конце игры.
