...
(async () => {
    ...
    try {
        ...
        conslole.warn(err);
        conslole.warn("Shutting down socket server...");
        process.exit(1);
    }
    ...
    *      - explCount --- last number of any explanation (from 1 to ...)
    ...
    class Room {
        ...
        async gamePrepare() {
            ...
            this.users.forEach(function (user) {
                if (user.ID) {
                    await runAsync(`INSERT INTO Participating
                        VALUES (?, ?)`,
                        [
                            this.gameID,
                            user.ID
    ...
    class Callbacks {
        static async joinRoomCallback(socket, data, err) {
            ...
            let gameID = -1;
            if (!(key in rooms)) {
                // TODO: Problems with asyncronous behaviour are possible.
                const resp = await runAsync("INSERT INTO Games(State, Host) VALUES (\"wait\", \"\");", []);
                if (resp === null) {
                    console.warn("Callbacks.joinRoomCallback: DB crash!"); // TODO smth with this crash
                }
